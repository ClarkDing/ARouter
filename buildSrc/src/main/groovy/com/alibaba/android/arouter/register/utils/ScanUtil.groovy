...
class ScanUtil {
    ...
    static void scanJar(File jarFile, File destFile) {
        if (jarFile) {
            ...
            Logger.i('scanJar jarFile: ' + jarFile.absolutePath)
            ...
            while (enumeration.hasMoreElements()) {
                ...
                Logger.i('scanJar jarEntry: ' + jarEntry.getName())
                String entryName = jarEntry.getName()
                if (entryName.startsWith(ScanSetting.ROUTER_CLASS_PACKAGE_NAME)) {
                    Logger.i('scanJar start: ' + entryName)
    ...
    static void scanClass(File file) {
        Logger.i('scanClass start')
        scanClass(new FileInputStream(file))
    }

    static void scanClass(InputStream inputStream) {
        Logger.i('scanClass final start')
